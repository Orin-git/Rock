#!/usr/bin/env python3
"""CLI entry: visual_db_capture_candidate."""
...
def main(argv=None) -> None:
    ...
    p.add_argument('--trigger-svc', action='store_true', help='Call Trigger service')
    args = p.parse_args(argv)

    if args.oneshot:
        raise SystemExit(_oneshot(args.dry_run))
    ...
    if args.trigger_svc:
        ...
        raise SystemExit(0 if fut.result().success else 1)

    raise SystemExit(_via_service(args.dry_run, args.timeout))
