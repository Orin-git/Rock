...
# nav_result_code values that mean "not now", never "this cell is unreachable".
# INTERLOCK_PAUSED  = localization/safety interlock cancelled the in-flight goal
...
class VisualDbBuildOrchestrator(Node):
    ...
    def _run_build(self) -> None:
        ...
        try:
            ...
            for round_i in range(max_rounds):
                ...
                # (cell, yaw_bin) already attempted this session. Without this the
                # proximity-scored planner refills the round budget with the cells
                # that just failed, the freshness filter then drops all of them,
                # and the session stops with hundreds of untouched gaps left.
                session_attempted_pairs: Set[Tuple[str, int]] = set()
                for _prev in self._session.goals:
                    _pc = str(_prev.get('spatial_cell') or '')
                    try:
                        _pb = int(_prev.get('yaw_bin'))
                ...
                if not fresh:
                    ...
                    if round_i == 0 and self._session.build_kind == 'CANDIDATE_ONLY':
                        ...
                        self._persist_session()
                        return
                    # Planner returned nothing new. If gaps remain, the round
                    # budget was starved rather than the map being done — say so
                    # loudly instead of stopping silently again.
                    actionable = set(only_cells or ()) - set(exclude_cells)
                    if actionable:
                        stop_reason = 'planner_starved'
                        self._dump_planner_starved(
                            vroot=vroot,
                            round_i=round_i,
                            goals=goals,
                            seen=seen,
                ...
                settle_after_interlock = 0.0
                # ★ Pause-EPISODE state. Round-scoped: the clock belongs to the
                # pause EPISODE, not to whichever goal happened to be at the head
                # of the queue when localization dropped.
                #
                # The old shape re-zeroed `pause_since` at the top of every goal
                # and cleared it the instant the pause loop exited -- and that
                # loop exited on a single `_should_pause() == False` SAMPLE, which
                # is a flicker, not a recovery (`_await_stable_localization`
                # measures 14/14 instant aborts firing into exactly such windows).
                # So `need_operator_grace_sec` -- the liveness guarantee that ends
                # a session which cannot make progress -- was unreachable on any
                # session whose localization cleared even momentarily. Such a
                # session ground on to `max_session` and was recorded as
                # `session_timeout`: the 90 s fast-fail silently degraded into
                # burning the whole budget under the wrong reason.
                #
                # The operative half of the fix is the settle guard inside the
                # pause loop below (an exit now requires the clear to HOLD for
                # `pause_clear_settle_sec`). Declaring the state here is the
                # structural half: every current exit path from that loop still
                # falls through the reset, so this is defensive rather than
                # load-bearing -- but it keeps the lifetime honest, so a future
                # `continue` or early `break` inside the loop cannot quietly
                # re-arm a fresh 90 s on every goal.
                #
                # `pause_clear_settle_sec` defaults to `interlock_settle_sec`
                # (3.0 s), not to a value invented here: that 3.0 s is the window
                # the codebase already derived from the measured 0.16-0.95 s
                ...
                while queue:
                    ...
                    while not self._stop.is_set():
                        if self._should_pause():
                            ...
                            # A clear SAMPLE, not a recovery. Hold until it has
                            # held for `pause_clear_settle_sec` before calling the
                            # episode over: a goal dispatched into a flicker is
                            # aborted by Nav2 in <1 s. The episode clock below
                            # deliberately keeps running across these samples, so
                            # flickering cannot buy the session another grace
                            # period.
                            if pause_clear_since <= 0.0:
                                pause_clear_since = time.monotonic()
                            if (time.monotonic() - pause_clear_since
                                    >= pause_clear_settle_sec):
                                break
                        ...
                        if time.time() - self._session.start_time > max_session:
                            ...
                            break
                        time.sleep(0.5)
                    # Reaching here means the episode ended because the clear was
                    # STABLE (or no episode ever opened) -- that is the one event
                    # that earns a fresh grace budget.
                    if pause_since > 0.0:
                        self._pause_since = 0.0
                        self._pause_reason = ''
                        pause_since = 0.0
                    ...
                    if settle_after_interlock > 0.0:
                        # ★ Bounded now. This call had no exit: if _should_pause()
                        # flickers faster than `settle_sec`, `ok_since` is reset
                        # before it can mature and the loop never returns. That is
                        # the 2026-09-14 shape (worker alive, session stuck in
                        # PAUSED, no way out but a manual stop) -- the pause loop
                        # got its exit then, this helper was missed.
                        if not self._await_stable_localization(
                                settle_after_interlock, need_operator_grace):
                            settle_after_interlock = 0.0
                            if self._stop.is_set():
                                stop_reason = 'user_stop'
                            else:
                                stop_reason = 'pause_timeout'
                                self._set_state('ABORTED', stop_reason)
                                self.get_logger().warn(
                                    f'localization would not hold clear for '
                                    f'{interlock_settle_sec:.1f}s -> ending session '
                                    f'(stop_reason={stop_reason}, will not promote)')
                            break
                        settle_after_interlock = 0.0
                        if self._stop.is_set():
                    ...
                    rec = self._session.goals[-1]
                    rec['nav_result_code'] = nav_code
                    # Duration is what separates a real navigation failure from a
                    # goal fired into a momentary localization gap; the ledger
                    # needs it to decide whether the CELL is at fault.
                    rec['nav_elapsed_sec'] = round(nav_elapsed, 3)
                    rec['nav_retryable'] = True
                    # A NAV2_STATUS abort that finished before navigation could
                    # have started is a bad start pose, not a verdict on the
                    # cell. Treat it like an interlock, not like a failure.
                    instant_abort = (
                        not nav_ok
                        and nav_elapsed < MIN_NAV_ATTEMPT_SEC
                        and str(nav_code).startswith('NAV2_STATUS:')
                    )
                    if not nav_ok and (nav_code in _CANCEL_NAV_CODES or instant_abort):
                        # Says nothing about the cell. Requeue at the tail so it
                        # is retried once localization is back; never billed as a
                        # failure, never written to the nav-fail ledger.
                        key = (str(g.spatial_cell), int(g.yaw_bin))
                        tries = interlock_retries.get(key, 0) + 1
                        interlock_retries[key] = tries
                        rec['nav'] = nav_code
                        rec['future_resume_candidate'] = False
                        rec['permanent_unreachable'] = False
                        if instant_abort:
                            rec['nav_note'] = 'instant_abort_bad_start_pose'
                        retry = (
                            nav_code != 'ABORTED'
                            and tries <= max_interlock_retries
                    ...
                    if not nav_ok:
                        ...
                        rec['nav'] = 'NAV_FAILED'
                        rec['future_resume_candidate'] = True
                        rec['permanent_unreachable'] = False
                        self._publish_status()
                        continue
    ...
    def _await_stable_localization(self, settle_sec: float,
                                   max_wait_sec: float = 0.0) -> bool:
        """Block until localization is un-paused continuously for settle_sec.

        _should_pause() flickers. A goal fired into one of those momentary
        "clear" windows is aborted by Nav2 within ~0.2 s (measured: 14/14 of the
        NAV2_STATUS:6 aborts in the 2026-09-10 session ran 0.16-0.95 s, versus
        >=5 s for all 54 real attempts). That instant abort is a bad start pose,
        not a reachability verdict, so do not dispatch into a flicker.

        ★ Returns True once the clear has held for `settle_sec`; False if
        `max_wait_sec` elapsed first, or the session was stopped.

        The bound is not optional in practice: without it, a `_should_pause()`
        that alternates faster than `settle_sec` resets `ok_since` on every true
        sample, so the loop never returns and the worker sits in PAUSED with a
        live session and no exit but a manual stop. That is precisely the
        2026-09-14 shape the pause loop was given an exit for; this helper was
        missed. Callers must treat False as "cannot make progress", never as
        "clear".
        """
        if settle_sec <= 0.0:
            return True
        ...
        deadline = ((time.monotonic() + max_wait_sec)
                    if max_wait_sec > 0.0 else None)
        while not self._stop.is_set():
            if self._should_pause():
                ok_since = None
                self._set_state('PAUSED', 'interlock_settle')
            else:
                if ok_since is None:
                    ok_since = time.monotonic()
                elif time.monotonic() - ok_since >= settle_sec:
                    return True
            if deadline is not None and time.monotonic() >= deadline:
                self.get_logger().warn(
                    f'localization never held clear for {settle_sec:.1f}s '
                    f'within {max_wait_sec:.0f}s -> cannot make progress')
                return False
            time.sleep(0.3)
        return False

    def _dump_planner_starved(
        self,
        *,
        vroot: Path,
        round_i: int,
        ...
        actionable: Set[Tuple[int, int]],
        exclude_cells: Set[Tuple[int, int]],
    ) -> None:
        """Persist why a round stopped while actionable gaps remained."""
        try:
            path = vroot / 'state' / f'diag_planner_starved_{int(time.time())}.json'
                ...
                json.dumps(
                    {
                        'at': time.time(),
                        'round': round_i,
                        'session_id': self._session.build_session_id if self._session else None,
                        'build_kind': self._session.build_kind if self._session else None,
                        'planned_goals': [g.as_dict() for g in goals],
                        'seen_pairs': sorted(f'{c}|{b}' for c, b in seen),
                        'actionable_gap_cells': sorted(f'cell_{cx}_{cy}' for cx, cy in actionable),
                        'exclude_cells': sorted(f'cell_{cx}_{cy}' for cx, cy in exclude_cells),
                    },
                    indent=2,
                    sort_keys=True,
                )
            ...
            self.get_logger().error(
                f'planner starved: {len(actionable)} actionable gaps left but no fresh goals; '
                f'diag -> {path}'
            )
        except Exception:  # noqa: BLE001
            self.get_logger().warn('planner_starved diagnostic dump failed')
    ...
    def _navigate_to(self, goal: PatrolGoal) -> Tuple[bool, str]:
        ...
        while not result_fut.done():
            ...
            # cancellations and never bills either of these as a cell failure.
            if self._stop.is_set():
                try:
                    gh.cancel_goal_async()
                except Exception:  # noqa: BLE001
