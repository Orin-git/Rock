                                      ...
                                      const AS_SDK_CAM_MODEL_E &cam_type)
{
    ...
    if (!reconnected) {
        ...
        case AS_SDK_CAM_MODEL_HP60CL1:
            ...
            stPublisherInfo.depth_points_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("~/depth" + std::to_string(
                                                   dev_idx) + "/points", sensor_qos);
            stPublisherInfo.mjpeg_raw_pub = this->create_publisher<sensor_msgs::msg::CompressedImage>("~/mjpeg" + std::to_string(
                                                dev_idx) + "/compressed", sensor_qos);
            break;
    ...
    for (auto it = imgPubList.begin(); it != imgPubList.end(); it++) {
        if (it->pCamera == pCamera) {
            ...
            if ((cam_type == AS_SDK_CAM_MODEL_HP60C) || (cam_type == AS_SDK_CAM_MODEL_HP60CN)
                ...
                || (cam_type == AS_SDK_CAM_MODEL_TANGGULA_B) || (cam_type == AS_SDK_CAM_MODEL_HP60CL1)) {
...
int CameraPublisher::genRosMjpegImage(const AS_Frame_s *pstFrame, sensor_msgs::msg::CompressedImage *pstImage)
{
        ...
        AS_CAM_Parameter_s &stParameter, builtin_interfaces::msg::Time time)
{
    ...
    auto &dc = m_depth_fps_counter[pCamera];
    auto sdk_now = std::chrono::steady_clock::now();
    if (!dc.initialized) {
        dc.last_time = sdk_now;
        dc.initialized = true;
    }
    dc.sdk_count++;
    ...
    for (auto it = imgPubList.begin(); it != imgPubList.end(); it++) {
        if (it->pCamera == pCamera) {
            ...
            if (rclcpp::ok()) {
                ...
                if (depth_published) {
                    double publish_ms = std::chrono::duration<double, std::milli>(now - publish_begin).count();
                    dc.pub_count++;
                    dc.publish_total_ms += publish_ms;
                    if (publish_ms > dc.publish_max_ms) {
                        dc.publish_max_ms = publish_ms;
                    }
                }
                double elapsed = std::chrono::duration<double>(now - dc.last_time).count();
                if (elapsed >= 1.0) {
                    double publish_avg_ms = dc.pub_count ? (dc.publish_total_ms / dc.pub_count) : 0.0;
                    RCLCPP_INFO(m_logger, "[FPS] depth sdk=%.1f pub=%.1f publish=%.2f/%.2f ms (size=%ux%u)",
                                dc.sdk_count / elapsed, dc.pub_count / elapsed,
                                publish_avg_ms, dc.publish_max_ms,
                                pstData->depthImg.width, pstData->depthImg.height);
                    dc.sdk_count = 0;
                    dc.pub_count = 0;
                    dc.publish_total_ms = 0.0;
                    dc.publish_max_ms = 0.0;
                    dc.last_time = now;
                }
            }
            break;
                                       ...
                                       AS_CAM_Parameter_s &stParameter, builtin_interfaces::msg::Time time)
{
    ...
    auto &rc = m_rgb_fps_counter[pCamera];
    auto sdk_now = std::chrono::steady_clock::now();
    if (!rc.initialized) {
        rc.last_time = sdk_now;
        rc.initialized = true;
    }
    rc.sdk_count++;
    ...
    if (rclcpp::ok()) {
        for (auto it = imgPubList.begin(); it != imgPubList.end(); it++) {
            if (it->pCamera == pCamera) {
                ...
                double publish_ms = std::chrono::duration<double, std::milli>(now - publish_begin).count();
                rc.pub_count++;
                rc.publish_total_ms += publish_ms;
                if (publish_ms > rc.publish_max_ms) {
                    rc.publish_max_ms = publish_ms;
                }
                double elapsed = std::chrono::duration<double>(now - rc.last_time).count();
                if (elapsed >= 1.0) {
                    double publish_avg_ms = rc.pub_count ? (rc.publish_total_ms / rc.pub_count) : 0.0;
                    RCLCPP_INFO(m_logger, "[FPS] rgb   sdk=%.1f pub=%.1f publish=%.2f/%.2f ms (size=%ux%u)",
                                rc.sdk_count / elapsed, rc.pub_count / elapsed,
                                publish_avg_ms, rc.publish_max_ms,
                                pstData->rgbImg.width, pstData->rgbImg.height);
                    rc.sdk_count = 0;
                    rc.pub_count = 0;
                    rc.publish_total_ms = 0.0;
                    rc.publish_max_ms = 0.0;
                    rc.last_time = now;
                }
            }
        }
                                       ...
                                       AS_CAM_Parameter_s &stParameter, builtin_interfaces::msg::Time time)
{
    ...
    RCLCPP_INFO_STREAM_ONCE(m_logger,"publish mjpeg");
    static unsigned int seq = 0;
    sensor_msgs::msg::CompressedImage mjpegImage;

    mjpegImage.header.stamp = time;

    genRosMjpegImage(&pstData->mjpegImg, &mjpegImage);
    seq++;
    ...
    if (rclcpp::ok()) {
        for (auto it = imgPubList.begin(); it != imgPubList.end(); it++) {
            if (it->pCamera == pCamera) {
                ...
                it->mjpeg_raw_pub->publish(mjpegImage);
            }
        }
        // ros::spinOnce();
    }
        ...
        if (it->pCamera == pCamera) {
            ...
            if (it_par != m_cam_type_map.end()) {
                if ((it_par->second == AS_SDK_CAM_MODEL_HP60C) || (it_par->second == AS_SDK_CAM_MODEL_HP60CN) ||
                    ...
                    (it_par->second == AS_SDK_CAM_MODEL_TANGGULA_B) || (it_par->second == AS_SDK_CAM_MODEL_CHANGJIANG_C) ||
                    (it_par->second == AS_SDK_CAM_MODEL_HP60CL1)) {
...
void CameraPublisher::getSubListStreamType(const PUBLISHER_INFO_S &publisherInfo, unsigned int &type)
{
    ...
    // mjpeg
    getSubTopicStreamType<sensor_msgs::msg::CompressedImage>(publisherInfo.mjpeg_raw_pub, streamType, publisherInfo.index);
    type |= streamType;
...
int CameraPublisher::streamController()
{
    ...
    while (rclcpp::ok()) {
        ...
        for (auto it = imgPubList.begin(); it != imgPubList.end(); it++) {
            ...
            getSubListStreamType(*it, target_type);
            if (target_type != it->stream_flg) {

                if (it->camStatus == CAMERA_CLOSED_STATUS) {
                    it->stream_flg = 0;
                    continue;
                }
                if (target_type == 0) {
                    if (it->camStatus == CAMERA_STREAM_STATUS) {
                        ret = AS_SDK_StopStream(it->pCamera);
                        if (ret == 0) {
                            it->stream_flg = 0;
                            it->camStatus = CAMERA_OPENED_STATUS;
                            onCameraStop(it->pCamera);
                        }
                    } else {
                        it->stream_flg = 0;
                    }
                    ...
                    if (it->camStatus == CAMERA_STREAM_STATUS) {
                        AS_SDK_StopStream(it->pCamera);
                        std::this_thread::sleep_for(std::chrono::milliseconds(100));
                    }
                    ret = AS_SDK_StartStream(it->pCamera, target_type);
                    if (ret == 0) {
                        if (it->stream_flg == 0) {
                            onCameraStart(it->pCamera);
                        }
                        it->stream_flg = target_type;
                        it->camStatus = CAMERA_STREAM_STATUS;
                        RCLCPP_INFO_STREAM(m_logger, "Stream updated to flag: 0x" << std::hex << target_type << std::dec <<  std::endl);
                    } else {
                        RCLCPP_ERROR_STREAM(m_logger, "Start stream failed with flag: 0x" << std::hex << target_type << std::dec <<  std::endl);
                    }
                }
            }
        }
