...
typedef struct CAM_FRAME {
    AS_FRAME_Type_e type = AS_FRAME_Type_e::AS_FRAME_TYPE_BUTT;
...
typedef struct STREAM_CALLBACK_S {
    AS_CAM_StreamCallback callback = nullptr;
...
typedef struct MERGE_CALLBACK_S {
    AS_CAM_MergeFrameCallback callback = nullptr;
    void *privateData = nullptr;
} AS_CAM_Merge_Cb_s;
...
typedef struct STREAM_PARAM_S {
    ...
    STREAM_PARAM_S(int width = 0, int height = 0, int fps = 0)
...
typedef struct CAM_UPGRADE_DEV_S {
    AS_CAM_UpGradeCallback callback = nullptr;
    void *privateData = nullptr;
...
typedef enum CAMERA_MODEL_E {
    ...
    AS_SDK_CAM_MODEL_CHANGJIANG_C,
    AS_SDK_CAM_MODEL_HP60PRO,
    AS_SDK_CAM_MODEL_HP60CL1,
...
typedef struct AS_LISTENER_CALLBACK {
    AS_Listener_Callback onAttached = nullptr;
    AS_Listener_Callback onDetached = nullptr;
    void *privateData = nullptr;
...
typedef struct AS_CONFIG_INFO {
    ...
    bool is_pclOrganized = false;
    bool depth_decimal = false;
} AS_CONFIG_INFO_S;