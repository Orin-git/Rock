...
typedef struct PUBLISHER_INFO {
    ...
    rclcpp::Publisher<sensor_msgs::msg::CompressedImage>::SharedPtr mjpeg_raw_pub;
...
private:
    ...
    int genRosMjpegImage(const AS_Frame_s *pstFrame, sensor_msgs::msg::CompressedImage *pstImage);
...
private:
    struct StreamFpsCounter {
        uint32_t sdk_count = 0;   /* frames delivered by SDK callback */
        uint32_t pub_count = 0;   /* frames actually published to ROS2 */
...
private:
    ...
    std::map<AS_CAM_PTR, StreamFpsCounter> m_depth_fps_counter;
    std::map<AS_CAM_PTR, StreamFpsCounter> m_rgb_fps_counter;
};
